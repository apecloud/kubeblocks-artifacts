    ...
    *,
    ...
    for size in range(max_size, min_size - 1, -2):
        ...
        if width <= x2 - x1 and height <= y2 - y1:
            ...
            draw.multiline_text(pos, text, font=font, fill=fill, spacing=spacing, align=align)
            return
...
def build_panel_1(cn: bool = False) -> Image.Image:
    ...
    if cn:
        draw_fit_text(draw, (110, 112, 780, 255), "好，小鸭。\n带我从头捋一遍。", 40, 28, cn=True)
        out = ROOT / "panel-1-v4-cn.png"
    else:
        draw_fit_text(draw, (110, 120, 830, 235), "Okay, duck. Walk me through it.", 48, 30)
...
def build_panel_2(cn: bool = False) -> Image.Image:
    ...
    if cn:
        draw_fit_text(draw, (110, 110, 745, 245), "请求进来，\n我存下来，然后——", 42, 28, cn=True)
        out = ROOT / "panel-2-v4-cn.png"
    else:
        draw_fit_text(draw, (110, 110, 765, 245), "Request comes in,\nI save it, then—", 44, 28)
...
def build_panel_3(cn: bool = False) -> Image.Image:
    ...
    if cn:
        draw_fit_text(draw, (135, 135, 500, 245), "……哦。", 54, 36, cn=True)
        out = ROOT / "panel-3-v4-cn.png"
    else:
        draw_fit_text(draw, (145, 135, 475, 245), "…oh.", 58, 38)
...
def build_panel_4(cn: bool = False) -> Image.Image:
    ...
    if cn:
        draw_fit_text(draw, (110, 120, 700, 230), "……这事别外传。", 46, 30, cn=True)
        draw_fit_text(draw, (110, 250, 790, 320), "睡眠：0 小时 · 鸭子：8 小时", 34, 22, cn=True)
        out = ROOT / "panel-4-v4-cn.png"
    else:
        draw_fit_text(draw, (110, 120, 720, 230), "…we don't tell anyone.", 46, 30)
        draw_fit_text(draw, (110, 250, 790, 320), "Sleep: 0h · Duck: 8h", 34, 22)
...
def main() -> None:
    ...
    build_comic(en_panels, "Rubber Duck Debugging", ROOT / "comic-v4.png", ROOT / "comic-v4-review.png")
    build_comic(cn_panels, "小黄鸭调试法", ROOT / "comic-v4-cn.png", ROOT / "comic-v4-review-cn.png", cn=True)
